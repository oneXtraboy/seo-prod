...
function normalizePath(pathname) {
  ...
  if (!value) return '/';
  const parsedValue = /^https?:\/\//i.test(value) ? new URL(value).pathname : value;
  const withLeadingSlash = parsedValue.startsWith('/') ? parsedValue : `/${parsedValue}`;
...
.container{max-width:1200px;margin:0 auto;padding:0 24px}.section-container{width:100%;max-width:1200px;margin:0 auto;padding:0 24px}.section-container-wide{max-width:1360px;padding:0 28px}.grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:24px}.cards-grid{display:grid;gap:var(--gap,var(--space-stack-md));width:100%;grid-template-columns:repeat(var(--cols,1),minmax(0,1fr))}.cards-grid>*{min-width:0}.grid-1-2-3{--cols:1;--cols-md:2;--cols-lg:3}.grid-1-2-4{--cols:1;--cols-md:2;--cols-lg:4}.grid-lg-3{--cols-lg:3}.section{padding:var(--s48) 0;border-bottom:1px solid var(--divider)}
...
.btn-primary{background:linear-gradient(180deg,#0A72D8,var(--accent));border-color:#0A72D8;color:var(--cta-text);text-shadow:0 1px 1px #00000033}.btn-primary:hover{background:linear-gradient(180deg,#0F7DE6,var(--accent-hover));border-color:#005ab3;color:var(--cta-text-hover)}.btn-primary:focus-visible{background:linear-gradient(180deg,#0F7DE6,var(--accent-hover));border-color:#005ab3;color:var(--cta-text-hover);outline-color:#99ccff}.btn-primary:active{background:linear-gradient(180deg,#005fbe,#004d99);border-color:#004a94;color:var(--cta-text-hover)}
...
.reviews-card-photo-placeholder{width:100%;height:100%;display:flex;align-items:center;justify-content:center;text-align:center;font-size:.8rem;line-height:1.3;color:var(--text3);padding:10px}
...
<body>
<header class="header"><div class="container header-in">${isHomePage(page.slug) ? `<span class="brand brand-current" aria-current="page">${escapeHtml(site.brandName || 'SEO-PROD')}</span>` : `<a class="brand brand-link" href="/">${escapeHtml(site.brandName || 'SEO-PROD')}</a>`}<nav class="nav">${renderHeaderNav(page.slug)}</nav></div></header>
${railHtml}
