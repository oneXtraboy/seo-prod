...
function renderLanding(page) {
  ...
  const utilityBar = data.utilityBar || {};
  const utilityBarText = utilityBar.text || 'Отвечаем в рабочее время, без спама и навязчивых продаж. Первый контакт — через brief, Telegram или email.';
  const utilityBarCta = utilityBar.cta || {};
  const utilityBarSection = `<section class="home-utility-bar" aria-label="Сервисная плашка"><div class="section-container home-utility-bar-in"><p class="home-utility-bar-copy">${escapeHtml(utilityBarText)}</p><a class="btn btn-primary home-utility-bar-cta" href="${escapeHtml(utilityBarCta.href || '/contact/')}">${escapeHtml(utilityBarCta.text || 'Оставить заявку')}</a></div></section>`;
  ...
  return utilityBarSection + hero + eeat + results + process + pricing + clients + reviews + team + faq + contact;
}
