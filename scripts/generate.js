...
function renderCaseDetailPage(page) {
  ...
  const businessProblemSection = (isCaseOne || businessProblem.length)
    ? section('case-business-problem', 'Проблема бизнеса', `<div class="card"><p>Бизнес уже вкладывал деньги в продвижение сайта через подрядчиков, которые не дали системного результата.</p><p><strong>Что было на старте</strong></p><ul>${businessProblem.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`, 'section-container')
    : '';
  ...
  ${isCaseOne ? businessProblemSection : ''}
  ${!isCaseOne && startingBase.length ? section('case-starting-base', 'База на старте', `<div class="card"><ul>${startingBase.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`, 'section-container') : ''}
