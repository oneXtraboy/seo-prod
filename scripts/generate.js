...
function renderCaseDetailPage(page) {
  ...
  ${section('case-actions', 'Что сделали', `<div class="card"><ul>${actions.map(renderListItem).join('')}</ul></div>`, 'section-container')}
  ...
  ${section('case-metrics', 'Метрики и подтверждение', `<div class="card">${metrics.length ? `<ul>${metrics.map(renderListItem).join('')}</ul>` : '<p>Публичные абсолютные цифры не раскрываются; эффект подтверждён в рабочей отчётности.</p>'}</div>`, 'section-container')}
