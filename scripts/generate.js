...
function renderLanding(page) {
  ...
  const packages = section('packages', 'Пакеты и состав работ', `<div class="cards-grid cards-grid--3lg" style="--cards-cols:1;--cards-cols-md:2;--cards-gap:24px;">${data.packages.map((p) => `<article class="card"><h3>${escapeHtml(p.name)}</h3><p>${escapeHtml(p.includes)}</p><p class="muted"><strong>Не входит:</strong> ${escapeHtml(p.excludes)}</p></article>`).join('')}</div>`, 'section-container');
  ...
  const process = section('process', 'Как работаем', `<div class="cards-grid" style="--cards-cols:1;--cards-cols-md:2;--cards-cols-lg:4;--cards-gap:24px;">${data.process.map((p, i) => `<article class="card"><p class="muted">Шаг ${i + 1}</p><h3>${escapeHtml(p.title)}</h3><p>${escapeHtml(p.text)}</p></article>`).join('')}</div>`, 'section-container');
