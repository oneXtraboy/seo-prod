...
- Legacy root HTML is still used by tools/build.ps1 and tools/gen-sitemap.ps1; retire those scripts before deleting the old HTML tree.
- The seo-deploy SSH user can write only the incoming directory; releases and live are root-owned, with sudo restricted to promote and rollback.
